          ...
          {pasos.map((p, i) => (
            <div key={p.id} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center">
                ...
                <span className="text-[9px] sm:text-[10px] mt-2 uppercase tracking-wider text-textLight/50 hidden sm:block">
                  {p.label}
                </span>
              </div>
              {i < pasos.length - 1 && (
                <div
                  className={`flex-1 h-0.5 mx-2 sm:mx-3 mt-[-14px] sm:mt-[-18px] rounded transition-colors ${
